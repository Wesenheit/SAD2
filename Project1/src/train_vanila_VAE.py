...
from VAE import GaussianVAE,log1p_layer,norm_layer
...
def train_custom_VAE(args):
    ...
    mean=np.mean(x_train.X.toarray())
    std=np.std(x_train.X.toarray())
    print("mean: {},std: {}".format(mean,std))
    VAE=GaussianVAE([5000,500,200,args.latent_size],[args.latent_size,200,500,5000],
        "cuda" if args.cuda else "cpu",scale=1,preprocess=norm_layer(mean,std))
    ...
    for i in range(args.num_epoche):
        ...
        for data_batch in data_loader_train:
            err+=svi.step(data_batch.X)
            err_kl+=VAE.kl_loss(data_batch.X).item()
        optim.step()
        if i%10==0:
            ...
            for data_batch in data_loader_test:
                err_test+=svi.evaluate_loss(data_batch.X)
                err_kl_test+=VAE.kl_loss(data_batch.X).item()
        ...
        if i%args.checkpoint==0 and i>0:
            print("Saving model!")
            VAE.save(name="vanila_VAE_{}.tc".format(args.latent_size))
    ...
    plt.tight_layout()
    plt.savefig("vanila_VAE_training_{}.png".format(args.latent_size))
    VAE.save(name="vanila_VAE_{}.tc".format(args.latent_size))

if __name__=="__main__":
    parser=argparse.ArgumentParser(description="SAD 2, Project nr 1, training script for gaussian vae")

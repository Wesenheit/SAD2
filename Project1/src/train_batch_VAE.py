...
from VAE import NegBinomBatchVAE,ZeroPoissBatchVAE,log1p_layer,norm_layer
...
def train_batch_VAE(args):
    ...
    VAE=NegBinomBatchVAE([5000,500,200,args.latent_size],[args.latent_size,100,100,5000],num_batches,
                device=device,preprocess=norm_layer(mean,std),gelu=True) # model parameters
    ...
    for i in range(args.num_epoche):
        ...
        for data_batch in data_loader_train:
            ...
            batches=nn.functional.one_hot(codes.long(),num_classes=num_batches)
            err+=svi.step(data_batch.layers["counts"],batches) 
        optim.step()
        if i%10==0:
            err_list_kl.append(VAE.kl_loss(data_batch.layers["counts"]).item())
            err_list.append(err/n)
            err_test=0
            for data_batch in data_loader_test:
                codes=torch.tensor(data_batch.obs["batch"].cat.codes,device=device)
                batches=nn.functional.one_hot(codes.long(),num_classes=num_batches)
                err_test+=svi.evaluate_loss(data_batch.layers["counts"],batches)
            print("epoche: {}, train loss: {}, test loss: {}".format(i,err/n,err_test/n_test))
            err_list_test.append(err_test/n_test)
            err_list_test_kl.append(VAE.kl_loss(data_batch.layers["counts"]).item())

        if i%args.checkpoint==0 and i>0:
            print("Saving model!")
            VAE.save(name="batch_VAE_{}.tc".format(args.latent_size))
    ...
    plt.tight_layout()
    plt.savefig("batch_VAE_training_{}.png".format(args.latent_size))
    VAE.save(name="batch_VAE_{}.tc".format(args.latent_size))

if __name__=="__main__":
    parser=argparse.ArgumentParser(description="SAD 2, Project nr 1, training script for batch adjusted VAE")
    ...
    train_batch_VAE(parser.parse_args())
